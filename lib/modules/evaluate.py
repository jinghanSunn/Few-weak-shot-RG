...
def convert_GT():
    ...
    {
        "images": {
            image_id: {width, height, path, anns:[[class_id, [x, y, x, y]], ...]}
        }, 
    ...
    """
    # class_name = ['1', '3.7', '4.1', '5', '5.1', '6', '7', '9', '10', '10.1', '11', '14', '15', '16', '17', '19', '22', '23', '24', '25', '27', '28', '29', '31', '33', '34', '35', '37', '37.1', '39', '40', '41', '42']
    class_name = ['3', '3.1', '3.4', '3.6', '4.2', '20', '38']
    # GT_path = '/jizhicfs/jhsun/data/FFAIRNew/train_lesion_info_convert_coco.pkl'
    GT_path = '/jizhicfs/jhsun/data/FFAIRNew/test_lesion_info_convert_coco.pkl'
    with open(GT_path, 'rb') as f:
        anns = pickle.load(f, encoding='bytes') 
    # cid2cls = dict(zip([int(c['class_id']) for c in list(anns['classes'])], [c['name'] for c in list(anns['classes'])]))
    cid2cls = dict(zip([int(c) for c in range(len(class_name))], [c for c in class_name]))
    res = {
        'obj_per_image': {},
        'counter_per_class': {},
        'counter_per_image': {}
    }
    for image_id in anns['images'].keys():
...
def calc_mAP(submit, GT, num_class, U_classes=[]):
    ...
    class_name = ['3', '3.1', '3.4', '3.6', '4.2', '20', '38']
    # print(submit['0'][0])
    # print(GT['obj_per_image']) # no resize
    # print(submit) # no resize?
    # print(xx.xx)
    gt_counter_per_class = GT["counter_per_class"]
    gt_counter_per_image = GT["counter_per_image"]
    # print(gt_counter_per_class)
    # print(gt_counter_per_image)

    # 转换submit的key成为class name
    new_submit = dict()
    for class_id in submit.keys():
        name = class_name[class_id-1]
        # submit.update({name:submit.pop(class_id)}) # -1是因为submit现在是0-33，0是background,1开始才是前景类别
        new_submit[name] = submit[class_id]
    submit = new_submit
    used = {}
    ...
    for class_name in gt_counter_per_class: # 分别对每个类别计算
        count_true_positives[class_name] = 0
        # ipdb.set_trace()
        if class_name not in submit:
            continue
        dr_data = submit[class_name] # get pred box for 1 class
        nd = len(dr_data)
        tp = [0] * nd # creates an array of zeros of size nd
        fp = [0] * nd
        for idx, detection in enumerate(dr_data):
            file_id = detection["image_id"] # {image_id: "123.jpg", bbox: [xmin, ymin, xmax, ymax], C: 1.0}
